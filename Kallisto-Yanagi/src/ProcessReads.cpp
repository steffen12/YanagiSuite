...
int ProcessBatchReads(KmerIndex& index, const ProgramOptions& opt, MinCollector& tc, std::vector<std::vector<int>> &batchCounts) {
  ...
  std::vector<std::pair<const char*, int>> readNames;
  readNames.reserve(limit/50);
...
void MasterProcessor::update(const std::vector<int>& c, const std::vector<std::string> &ecReads, const std::vector<std::vector<int> > &newEcs, 
...
ReadProcessor::ReadProcessor(const KmerIndex& index, const ProgramOptions& opt, const MinCollector& tc, MasterProcessor& mp, int _id) :
 paired(!opt.single_end), tc(tc), index(index), mp(mp), id(_id) {
   ...
   readNames.reserve(bufsize/50);
   if (opt.umi) {
    umis.reserve(bufsize/50);
   }
   newEcs.reserve(1000);
   counts.reserve((int) (tc.counts.size() * 1.25));
   ecReads.reserve((int) (tc.counts.size() * 1.25));
   clear();
}
...
ReadProcessor::ReadProcessor(ReadProcessor && o) :
  ...
  readNames(std::move(o.readNames)),
  ...
  while (true) {
    ...
    if (mp.opt.batch_mode) {
      if (batchSR.empty()) {
        ...
        batchSR.fetchSequences(buffer, bufsize, seqs, names, quals, umis, readNames, readbatch_id, mp.opt.pseudobam );
      }
    } else {
      ...
      if (mp.SR.empty()) {
        ...
        // get new sequences
        mp.SR.fetchSequences(buffer, bufsize, seqs, names, quals, umis, readNames, readbatch_id, mp.opt.pseudobam || mp.opt.fusion);
      }
    ...
    mp.update(counts, ecReads, newEcs, ec_umi, new_ec_umi, paired ? seqs.size()/2 : seqs.size(), flens, bias5, pseudobatch, id);
    clear();
  }
}
    ...
    if (!u.empty()) {
      ...
      if (!mp.opt.umi) {
        ...
        if (ec == -1 || ec >= counts.size()) {
          ...
          // add to count vector
          ++counts[ec];
          std::string ecReadPointer;
          if(counts[ec] == 1) {
            ecReadPointer = "";
          } else {
            ecReadPointer = ecReads[ec];
          }
          std::string concatString = ecReadPointer + readName;
          ecReads[ec] = concatString;
          //std::cout << concatString << std::endl;
          //std::cout << ecReadPointer << std::endl;
          //std::cout << readName << std::endl << std::endl;
        }
      } else {       
        if (ec == -1 || ec >= counts.size()) {
  ...
  while (true) {
    ...
    if (mp.opt.batch_mode) {
      ...
      if (batchSR.empty()) {
        ...
        batchSR.fetchSequences(buffer, bufsize, seqs, names, quals, umis, readNames, readbatch_id, true );
      ...
      if (mp.SR.empty()) {
        ...
        // get new sequences
        mp.SR.fetchSequences(buffer, bufsize, seqs, names, quals, umis, readNames, readbatch_id, true);
        readPseudoAlignmentBatch(mp.pseudobatchf_in, pseudobatch);
        assert(pseudobatch.batch_id == readbatch_id);
        assert(pseudobatch.aln.size() == ((paired) ? seqs.size()/2 : seqs.size())); // sanity checks
      }
...
bool SequenceReader::fetchSequences(char *buf, const int limit, std::vector<std::pair<const char *, int> > &seqs,
  ...
  std::vector<std::pair<std::string, int> > &readNames,
  int& read_id, bool full) {
  ...
  while (true) {
    ...
    if (l1 > 0 && (!paired || l2 > 0)) {
      ...
      if (bufpos+bufadd< limit) {
        ...
        char * seq1Name = seq1->name.s;
        std::string s(seq1Name); 
        std::string seq1Name_ = seq1Name;
        readNames.emplace_back(seq1Name_ + ",", seq1->name.l);
        ...
        if (paired) {
          ...
          readNames.emplace_back(seq2->name.s, seq2->name.l);
          if (full) {
            p2 = buf+bufpos;
